...
real(dp), parameter :: pirr = d2r / 3600._dp
...
real(dp), dimension(nef), parameter :: ce = ci * d2r
...
real(dp), dimension(nob), parameter :: cob = cib * d2r
...
real(dp), dimension(nop), parameter :: cop = cip * d2r
...
subroutine calcorbitpars(start_year,cnt_year,orbit)
...
! Short cut to generate calc_year integrated with gwgen (Leo O Lai, 19 Apr 2021)
cal_year = start_year + cnt_year - 1
